...
def chi_square_test(data_count, expected_counts):
    ...
    chi_square_val = 0  # chi square test statistic
    for data, expected in zip(data_count, expected_counts):
        chi_square = math.pow(data - expected, 2)
        chi_square_val += chi_square / expected
    print("\nChi-squared Test Statistic = {:.3f}".format(chi_square_val))
    print("Critical value at a P-value of 0.05 is 15.51.")    
    return chi_square_val < 15.51
...
def bar_chart(data_pct):
    ...
    ax.scatter(index, BENFORD, s=150, c='blue', zorder=2, label='Benford')
